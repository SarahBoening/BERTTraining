...
class TextDataset(Dataset):
    def __init__(self, tokenizer, file_path='train', block_size=512):
        ...
        logger.info("checking if cached file exists: %s" % cached_features_file)
        if os.path.exists(cached_features_file):
            logger.info("Loading features from cached file %s", cached_features_file)
            with open(cached_features_file, 'rb') as handle:
                self.examples = pickle.load(handle)
        else:
            logger.warning("Creating features from dataset file at %s", file_path)
...
def load_and_cache_examples_multiprocess(args, file_list, tokenizer, evaluate=False):
    process_cnt = args.pre_process_count
    files_per_process = int(len(file_list) / process_cnt)
    remainder = int(len(file_list) % process_cnt)
    distribution = [files_per_process] * process_cnt
    ...
    with multiprocessing.Pool(processes=process_cnt) as pool:
        for process_id in range(process_cnt):
            sum_of_prev_processes = sum(distribution[:process_id])
            start = sum_of_prev_processes
            end = start + distribution[process_id]

            process = pool.apply_async(load_and_cache_examples_subprocess,
                                       args=(process_id, file_list, start, end, args, tokenizer))
...
def train(args, train_datasets, model, tokenizer, global_step, tr_loss, logging_loss, valid=None):
    ...
    logger.info("num of examples in first dataset: %d" % len(train_dataset.examples))

    for ds in train_datasets[1:]:
        train_dataset.join(ds)
    logger.info("num of examples after join: %d" % len(train_dataset.examples))
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    if args.local_rank == -1 and args.evaluate_during_training:  # Only evaluate when single GPU otherwise metrics may not average well
                        results = evaluate(args, model, valid, tokenizer)
                        if results["perplexity"] < best_pplxty:
...
def main():
    ...
    parser.add_argument("--gpu_ids", default="0", type=str,
                        help="ids of visible GPUs default=0 (str separated by comma)")

    parser.add_argument("--pre_process_count", default=2, type=int,
                        help="num of processes to preprocess the dataset")
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            model.to(args.device)
            result = evaluate(args, model, valid_file, tokenizer, prefix=prefix)
